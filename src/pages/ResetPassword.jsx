...
const ResetPassword = () => {
  ...
  useEffect(() => {
    if (!token) {
      setErrorMessage('Token de recuperación no válido o ausente.')
    }
  }, [token])
        ...
        <div translate="no" className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:p-10">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              <span>Escribe tu nueva contraseña para actualizar tu acceso.</span>
            </p>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-slate-600">
            <Link to="/login" className="font-medium text-slate-600 transition hover:text-slate-900">
              <span>← Cancelar y volver al Iniciar Sesión</span>
            </Link>
          </p>
        </div>
      </div>
